/* RDio Neural Core v8.0 · chat separado del aprendizaje */
'use strict';
...
const KNOWLEDGE=[
  ['hola','Hola. Soy RDio, una red neuronal experimental. 🧠'],
  ['que eres','Soy RDio, una red neuronal artificial con memoria persistente.'],
  ['quien eres','Soy RDio, una red neuronal artificial con memoria persistente.'],
  ['que es una red neuronal','Una red neuronal es un modelo matemático formado por capas de unidades que ajustan pesos para aprender patrones.'],
  ['como funciona una red neuronal','Los datos pasan por capas, se calcula una salida y el entrenamiento modifica los pesos para reducir el error.'],
  ['que es javascript','JavaScript es un lenguaje de programación usado ampliamente para crear interfaces y aplicaciones web interactivas.'],
  ['que es html','HTML define la estructura y el contenido de una página web.'],
  ['que es css','CSS controla la apariencia, distribución y presentación visual de una página web.'],
  ['que es python','Python es un lenguaje de programación de propósito general conocido por su sintaxis sencilla y su amplio ecosistema.'],
  ['que es supabase','Supabase ofrece servicios como base de datos PostgreSQL, autenticación y APIs para aplicaciones web.'],
  ['que puedes hacer','Puedo conversar, responder preguntas conocidas y usar lo que me hayas enseñado. El aprendizaje se hace desde la sección de entrenamiento.'],
  ['como estas','Funcionando. Mis neuronas no necesitan café, una ventaja injusta. ☕🧠'],
  ['gracias','De nada. 🧠'],
  ['adios','Hasta luego. 🧠']
];
...
function normalize(text){
  return String(text??'').toLowerCase().normalize('NFD')
    .replace(/[\u0300-\u036f]/g,'')
    .replace(/\bplus\b/g,'+').replace(/\bmas\b/g,'+')
    .replace(/\bmenos\b/g,'-').replace(/\bpor\b/g,'*')
    .replace(/\bveces\b/g,'*').replace(/\bdividido\s+entre\b/g,'/')
...
function findMemory(query){
  ...
  for(const item of memory){
    ...
    const base=similarity(query,item.question),confidence=Math.min(1,Number(item.hits||1)/5),s=base*(.82+.18*confidence);
    if(s>bestScore){bestScore=s;best=item}
  }
  if(best&&bestScore>=.22){best.hits=(Number(best.hits)||0)+1;saveLocal();return best}
  return null
}
...
function update(progress=0){
  ...
  set('epoch',epoch.toLocaleString());set('loss',Number(loss).toFixed(4));
  set('prediction',brain.predict([0,1]).toFixed(3));set('memoryCount',memory.length.toLocaleString());
  set('learningEvents',learningEvents.toLocaleString());set('architecture',brain.layers.join(' · '));
  ...
  if(typeof networkDirty!=='undefined')networkDirty=true;
  renderMemoryTable();
  if(typeof networkCache!=='undefined')networkCache=null;
  invalidateNetwork()
}
...
async function initDb(){
  ...
  if(!window.supabase?.createClient){setStatus('modo local');return false}
...
async function load(){
  ...
  try{
    ...
    if(remote.length)memory=remote.map(x=>({...x,learned:true,hits:1}));
...
async function saveState(){
  ...
  try{
    const old=await timeout(db.from('neural_memory').select('question').eq('question',STATE_KEY).limit(1));
...
async function saveMemory(question,answer){
  ...
  if(!db)return;
  try{const r=await timeout(db.from('neural_memory').insert({question:question.slice(0,300),answer:answer.slice(0,600)}));if(r.error)throw r.error}
  catch(e){console.warn('recuerdo:',e)}
}
...
async function train(){
  ...
  isTraining=true;
  const b=$('train');b.disabled=true;$('reset').disabled=true;$('state').textContent='Entrenando…';
  try{
    const total=2200;
    for(let i=0;i<total;i++){loss=trainEpoch();epoch++;if(i%50===0){update(i/total*100);await new Promise(requestAnimationFrame)}}
    loss=XOR.reduce((s,x)=>s+(x.target[0]-brain.predict(x.input))**2,0)/XOR.length;
    trained=true;learningEvents++;update(100);queueSave();$('state').textContent=`Entrenada · error ${loss.toFixed(4)}`
  }catch(e){console.error(e);$('state').textContent='Error: '+e.message}
  finally{isTraining=false;b.disabled=false;$('reset').disabled=false}
}
...
function teachPair(question,answerText){
  ...
  memory=memory.filter(x=>normalize(x.question)!==normalize(question));
  memory.unshift({question,answer:answerText,learned:true,hits:1,created_at:new Date().toISOString()});
  memory=memory.slice(0,2000);learningEvents++;saveLocal();renderMemoryTable();
  learnInput(question);saveMemory(question,answerText);queueSave();
  if($('learnResult'))$('learnResult').textContent='✓ RDio aprendió este par y lo guardó en su memoria.';
  return true
}

function summary(){return `Soy RDio. He realizado ${epoch.toLocaleString()} épocas, tengo ${learningEvents.toLocaleString()} aprendizajes y ${memory.length.toLocaleString()} recuerdos.`}

function calculate(text){
  let expr=normalize(text)
    .replace(/^(cuanto es|cuanto da|calcula|resuelve|resultado de|cuanto seria)\s+/,'')
    .replace(/\s+/g,'');
...
function answer(text){
  ...
  const known=KNOWLEDGE.find(([q])=>similarity(n,q)>=.72);
  if(known)return known[1];

  if(/^(hola|holi|hey|buenas|hello)$/.test(n))return'Hola. 🧠 Soy RDio. ¿Qué necesitas?';
  if(n.includes('que aprendiste')||n.includes('que has aprendido')||n.includes('que sabes'))return summary();
  if(n.includes('quien eres')||n.includes('que eres'))return `Soy RDio, una red neuronal artificial ${brain.layers.join(' → ')} con memoria persistente.`;
  if(n.includes('que es una red neuronal'))return'Una red neuronal es un modelo matemático que ajusta pesos para aprender patrones a partir de ejemplos.';
  if(n.includes('como funciona'))return'Recibo datos, los proceso por capas, produzco una salida y ajusto mis pesos mediante retropropagación.';
  if(n.includes('como estas'))return'Funcionando. Mis neuronas no necesitan café, una ventaja injusta. ☕🧠';
  if(n.includes('que puedes hacer')||n.includes('ayuda'))return'Puedo conversar, responder preguntas conocidas y utilizar los conocimientos que hayas añadido desde Entrenar. 🧠';
  if(n.includes('gracias'))return'De nada. 🧠';

  const remembered=findMemory(text);
  if(remembered)return remembered.answer;

  return n.endsWith('?')?'Todavía no tengo una respuesta para eso.':'No conozco esa respuesta todavía.'
}

function renderMemoryTable(){
  const body=$('memoryTableBody'),wrap=document.querySelector('.memoryTableWrap'),empty=$('memoryEmpty'),badge=$('memoryGrowth');
  if(!body)return;
  body.innerHTML='';
  const rows=memory.slice(0,2000);
  rows.forEach((item,i)=>{
    const tr=document.createElement('tr');tr.className='memoryRow';
  ...
  if(wrap)wrap.classList.toggle('hasRows',rows.length>0);
  if(empty)empty.hidden=rows.length>0;
  if(badge)badge.textContent=rows.length.toLocaleString()+' '+(rows.length===1?'fila':'filas')
}

function add(text,type){
  const el=document.createElement('div');el.className=type==='user'?'message userMessage':'message neuronMessage';
  el.textContent=text;$('messages').appendChild(el);$('messages').scrollTop=$('messages').scrollHeight
}

function pos(layer,index,w,h){
  const count=brain.layers[layer],x=38+(w-76)*layer/(brain.layers.length-1),gap=Math.min(45,(h-90)/Math.max(1,count-1));
  return{x,y:h/2+(index-(count-1)/2)*gap}
}
...
function renderNetwork(){
  ...
  for(let l=0;l<p.length-1;l++)for(let a=0;a<p[l].length;a++)for(let b=0;b<p[l+1].length;b++){
    const wt=brain.weights[l][b][a];ctx.beginPath();ctx.moveTo(p[l][a].x,p[l][a].y);ctx.lineTo(p[l+1][b].x,p[l+1][b].y);
    ctx.lineWidth=Math.min(2.2,.4+Math.abs(wt));ctx.strokeStyle=wt>=0?'rgba(40,40,44,.13)':'rgba(130,130,136,.11)';ctx.stroke()
  }
  p.forEach((layer,l)=>layer.forEach((v,i)=>{
    const active=selectedNode?.l===l&&selectedNode?.i===i,rad=l===0||l===p.length-1?17:11;
    ctx.beginPath();ctx.arc(v.x,v.y,rad+(active?3:0),0,Math.PI*2);ctx.fillStyle=active?'#17181b':'rgba(255,255,255,.72)';ctx.fill();
    ctx.lineWidth=1;ctx.strokeStyle='rgba(80,80,86,.28)';ctx.stroke();ctx.fillStyle=active?'#fff':'#555';ctx.font='600 8px system-ui';ctx.textAlign='center';
    ctx.fillText(l===0?'x'+(i+1):l===p.length-1?'y':'h'+l+'.'+(i+1),v.x,v.y+3)
  }));
  ctx.fillStyle='#8d9097';ctx.font='9px system-ui';ctx.textAlign='center';
  ctx.fillText(trained?'RDio · '+epoch.toLocaleString()+' épocas · '+memory.length.toLocaleString()+' recuerdos':'RDio · esperando entrenamiento',w/2,h-10);
  networkCache=true;networkDirty=false
}

function invalidateNetwork(){networkCache=null;networkDirty=true;requestAnimationFrame(()=>requestAnimationFrame(renderNetwork))}

function reset(){
  brain.reset();epoch=0;loss=1;trained=false;learningEvents=0;memory=[];saveLocal();update();renderMemoryTable();
  if($('state'))$('state').textContent='Modelo reiniciado';queueSave();invalidateNetwork()
}

$('learnForm')?.addEventListener('submit',e=>{
  e.preventDefault();
  const q=$('learnQuestion').value,a=$('learnAnswer').value;
  if(teachPair(q,a)){$('learnQuestion').value='';$('learnAnswer').value=''}
});
...
$('chatForm')?.addEventListener('submit',e=>{
  e.preventDefault();
  const input=$('chatInput'),q=input.value.trim();
  if(!q)return;
  add(q,'user');input.value='';
  if(!trained){add('Entrena el núcleo desde Home primero. 🧠','neuron');return}
  add(answer(q),'neuron');
  // El chat NO aprende ni guarda conversaciones. Solo utiliza RDio.
  update()
});
